...
_WIDGETS = {"turn", "resources", "log", "prompt"}
...
def _hud(parsed: ParsedFile, _registries: Registries | None) -> list[ContentError]:
    ...
    for index, widget in enumerate(widgets):
        if not isinstance(widget, dict) or widget.get("type") not in _WIDGETS or widget.get("slot") not in _SLOTS:
            errors.append(parsed.error("must use a known widget type and slot", field=("widgets", index), expected="type: turn/resources/log/prompt and slot: top/side/bottom")); continue
