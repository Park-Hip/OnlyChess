...
- Did a public lifecycle stage, invariant, or current-wave boundary change? If so, update
  [status.md](status.md).
- Is the decision recorded once in the relevant spec or ADR rather than copied into a
...
Keep decision rationale in the relevant ADR. Keep source comments focused on the local constraint a
reader needs to preserve.
